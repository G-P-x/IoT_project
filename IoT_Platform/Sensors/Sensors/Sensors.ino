...
/************ BUILD SENSOR JSON ************/
// Tutti i sensori usano lo stesso percorso: buildResponse() gestisce
// internamente il campo threshold (valore se ok, null se non risponde).
...
void publishOnTopic(const char* topic, String sensors_list) {
  // Le soglie sono embedded in ogni record (campo threshold).
  // Il gateway non ha bisogno di un blocco thresholds separato.
  ...
  if (sensors_list.length() == 0) {
    for (int i = 0; i < SENSOR_COUNT; i++) {
      ...
      if (!first) json += ",";
      json += buildSensorJson(sensors[i], value, msg, ok);
      first = false;
    }
  } else {
    int start = 0;
    while (start < (int)sensors_list.length()) {
      ...
      if (s == nullptr) {
        ...
        json += buildSensorJson(s, value, msg, ok);
      }
      first = false;
    }
  }
