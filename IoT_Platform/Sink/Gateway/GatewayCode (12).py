...
# ================= LOAD CONFIG =================
# Modifica config.json per cambiare IP, porte o finestra dati.
# Nessuna modifica a questo file e' necessaria.
...
# Errori hardware noti: non generano allarme, ma vengono comunque loggati.
IGNORED_ERRORS = [
...
def _now() -> str:
    """Timestamp corrente in formato ISO-8601 UTC (es. 2026-05-05T14:30:00.000Z)."""
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # Ogni response dell'ESP diventa un record {time_stamp, record}.
        # Il campo threshold e' gia' embedded in ogni record dall'ESP.
        now = _now()
        records = []
        for r in decoded.get("responses", []):
            records.append({"time_stamp": now, "record": r})

        # Risposta a cmd_01: segnala al thread HTTP in attesa
        ...
        # Dati periodici: aggiorna cache e sliding window
        with _lock:
        ...
        # Anomalie reali (escludi errori hardware noti)
        anomalies = [
            r for r in records
...
def receive_command():
    ...
    # Tutti gli altri comandi (alarm, ecc.) vengono inoltrati direttamente
    _mqtt_client.publish(TOPIC_PUB, json.dumps(cmd))
    print(f"[MQTT OUT] {TOPIC_PUB} -> {cmd}")
...
def get_data():
    """Sliding window completa (max WINDOW_SIZE record)."""
    return jsonify(list(data_window))


@app.route("/anomalies", methods=["GET"])
def get_anomalies():
    """Anomalie reali: status=ERROR esclusi gli errori hardware noti."""
    return jsonify([
...
def get_critical():
    """Solo record con severity=critical."""
    return jsonify([
        r for r in data_window
