...
def get_dt_id_by_name(name: str) -> str:
    """Retrieve a single DT manifest by name."""
    dt_factory : DTFactory = current_app.config["DT_FACTORY"]
    dt = dt_factory.get_dt(dt_name= name)
    return dt.get("_id", "ERROR")
...
@dt_api.route("/<dt_name>/services", methods=["POST"])
def add_service_to_dt(dt_name):
    """
    ...
    try:
        ...
        dt_factory : DTFactory =current_app.config["DT_FACTORY"]
        dt_id = get_dt_id_by_name(name=dt_name)
        if dt_id == "ERROR":
            return jsonify({"error": f"missing DT with name: {dt_name}"}), 404
        dt_factory.add_service(
            dt_id=get_dt_id_by_name(name=dt_name),
...
@dt_api.route("/<dt_name>/services/<service_name>", methods=["DELETE"])
def remove_service_from_dt(dt_name, service_name):
    """
    ...
    try:
        ...
        if dt_id == "ERROR":
            return jsonify({"error": f"missing DT with name: {dt_name}"}), 404
        dt_factory.remove_service(dt_id, service_name)
...
#     try:
#         db_service : DatabaseService = current_app.config["DB_SERVICE"]
...
def register_dt_api_blueprints(app : Flask):
    """
